...
ADAPTER_DIR = os.path.join(SCRIPTS_DIR, "lora_adapters_extended")
...
def train_adapter(model, train_loader, archive_name, epochs=TRAIN_EPOCHS):
    ...
    for epoch in range(epochs):
        ...
        logger.info(f"{archive_name} Epoch {epoch+1}: avg loss = {avg_loss:.4f}, lr = {current_lr:.2e}")
    return model
...
def main():
    ...
    for name, imgs in data.items():
        train_dsets[name] = ArchiveDataset(imgs, train=True)
        eval_dsets[name] = ArchiveDataset(imgs, train=False)
        train_loaders[name] = DataLoader(train_dsets[name], batch_size=BATCH_SIZE, shuffle=True, num_workers=2)
        eval_loaders[name] = DataLoader(eval_dsets[name], batch_size=BATCH_SIZE_EVAL, shuffle=False, num_workers=2)
    ...
    for name in archive_names:
        results = compute_mae_mse(base_model, eval_loaders[name], DEVICE)
    ...
    for train_name in archive_names:
        logger.info(f"\n=== Training LoRA (r=64, α=128, 50 epochs) on {train_name} ===")
        model = create_lora_model().to(DEVICE)
        model = train_adapter(model, train_loaders[train_name], train_name)
        ...
        for eval_name in archive_names:
            results = compute_mae_mse(model, eval_loaders[eval_name], DEVICE)
            mean_mse = np.mean([r['mse'] for r in results])
    ...
    # ---- ΔMSE matrix ----
    logger.info("\n=== Pairwise ΔMSE Matrix (Extended r=64, 50 epochs) ===")
    ...
    suffix = "_extended"
    df.to_csv(os.path.join(ANALYSIS_DIR, f"lora_pairwise_mse{suffix}.csv"), index=False)
    mse_mat.to_csv(os.path.join(ANALYSIS_DIR, f"lora_mse_matrix{suffix}.csv"))
    dmse_mat.to_csv(os.path.join(ANALYSIS_DIR, f"lora_dmse_matrix{suffix}.csv"))
    results_dict = {
        'config': {'r': 64, 'alpha': 128, 'epochs': 50, 'augmentation': True, 'lr_scheduler': 'CosineAnnealing'},
    ...
    with open(os.path.join(ANALYSIS_DIR, f"lora_results{suffix}.json"), 'w') as f:
    ...
    logger.info(f"\n{dmse_mat.to_string()}")
    logger.info(f"\nResults saved to {ANALYSIS_DIR}/lora_*{suffix}.*")
